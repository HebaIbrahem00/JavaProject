                ...
                if (listen.startsWith(Protocol.INVITATION)) {
                    ...
                    Platform.runLater(() -> {
                        ...
                        try {
                            ...
                            connect("O", onlineStage, Xscene, root, ClientSocket.client);
        ...
        public void invitationDetails(String title, String header, String content, Stage s, Scene XOscene, String from) throws IOException, InterruptedException, Exception {
            ...
            if (result.get() == approve) {
                ...
                Scene Xscene = null;
                       Platform.runLater(() -> {
                        try {
                            connect("X", onlineStage, Xscene, root, ClientSocket.client);
                        } catch (IOException ex) {
                            Logger.getLogger(DisplayUsersController.class.getName()).log(Level.SEVERE, null, ex);
                        }
...
//                listen = ClientSocket.fromServer.readLine();
//                if (listen.startsWith(Protocol.CONNECTED)) {
//                    Platform.runLater(() -> {
//                        try {
//                            connect("O", onlineStage, Xscene, root, ClientSocket.client);
//                        } catch (IOException ex) {
...
//                onlineStage.show();
    ...
    private void TicTacLogic() {

    }

    private static void connect(String s, Stage stage, Scene sc, Parent parent, Socket se) throws IOException {
        TicTacGUI client = new TicTacGUI(s, stage, sc, parent, se);
        client.run();
    }
